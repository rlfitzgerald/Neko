...
{  
    try
    {
        ...
        if (parser.option("t") || parser.option("cross-validate"))
        {
            ...
            // ANDREW
            cout << "Image scanner levels = " << scanner.get_max_pyramid_levels()<<endl;
            scanner.set_min_pyramid_layer_size(8,8);
            //scanner.set_cell_size(6);
        ...
        {  
            ...
            if (!fin)
            {
                cout << "Can't find a trained object detector file " << svmArr[i]  << endl;
        ...
        // ANDREW
        //cout << "Detector window = " << detector.get_scanner().get_detection_window_width() <<"x"<<detector.get_scanner().get_detection_window_height()<<endl;
        cout << "Detector window = " << my_detectors[0].get_scanner().get_detection_window_width() <<"x"<<my_detectors[0].get_scanner().get_detection_window_height()<<endl;
        ...
        if (tolower(right_substr(parser[0],".")) == "xml")
        {
            ...
            {
                for(int i=0;i<NUM_DETECTORS;i++){
                cout << "Testing detector " << i << " on data..." << endl;
                cout << "Results (precision,recall,AP): " << test_object_detection_function(my_detectors[i], images, object_locations, ignore) << endl;
                }
                return EXIT_SUCCESS;
            }
        }
        ...
        for(int i=0;i<NUM_DETECTORS;i++){
            image_window hogwin(draw_fhog(my_detectors[i]), "Learned fHOG detector");
        }
        ...
        for (unsigned long i = 0; i < images.size(); ++i)
        {
            ...
            // ANDREW
            const std::vector<rectangle> rects = evaluate_detectors(my_detectors,images[i],-0.4);
